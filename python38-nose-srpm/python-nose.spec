...
grep -rl -e '^#!/usr/bin/env python$' -e '^#!/usr/bin/env python $' python3/ | \
    grep '\.py$' | \
    while read name; do
...
grep -rl -e '^#!/usr/bin/python$' -e '^#!/usr/bin/python $' python3/ | \
    grep '\.py$' | \
    while read name; do
...
if [ "%{__python3}" != "/usr/bin/python3" ]; then
    grep -rl -e '^#!/usr/bin/python3$' -e '^#!/usr/bin/python3 $' python3/ | \
	grep '\.py$' | \
	while read name; do
